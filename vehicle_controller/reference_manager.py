...
class ReferenceManager:
    def __init__(
        ...
        self.speed_profile_file = Path(speed_profile_file) if speed_profile_file else None
    ...
    def _load_speed_profile(self, n_points: int) -> np.ndarray:
        if self.speed_mode == "constant" or self.speed_profile_file is None:
            return np.full(n_points, self.constant_speed, dtype=float)

        mat = loadmat(self.speed_profile_file)
        if "pathv_ref" not in mat:
            raise KeyError(f"Speed profile file must contain pathv_ref: {self.speed_profile_file}")
